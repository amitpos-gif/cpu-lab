...
BEGIN
	...
	G1 :
    if (WORD_GRANULARITY = True) generate     -- each WORD has a unique address
        dtcm_addr_w <= mem_alu_res_w(DTCM_ADDR_WIDTH+1 DOWNTO 2);  -- drop 2 byte-offset bits
    elsif (WORD_GRANULARITY = False) generate -- each BYTE has a unique address
        dtcm_addr_w <= mem_alu_res_w(DTCM_ADDR_WIDTH-1 DOWNTO 0);
    end generate;
    ...
    MEM : dmemory
        ...
        port map (
            clk_i           => clk_i,
            rst_i           => rst_i,
            dtcm_addr_i     => dtcm_addr_w,            -- <-- now uses the granularity-aware signal
