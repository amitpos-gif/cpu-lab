...
----------------------------------------------------
entity tb_control_jc_no_carry is
end entity tb_control_jc_no_carry;
----------------------------------------------------

architecture sim of tb_control_jc_no_carry is
...
begin
    ...
    begin
        ...
        assert PCin = '0'
            report "FETCH failed: PCin should be 0 in FETCH"
            severity error;


        ----------------------------------------------------------------
        -- Step 2: JC instruction with Cflag = 0
        -- Expected: no jump, PCsel should be PC+1 = 10
        ----------------------------------------------------------------
        jc_s  <= '1';
        Cflag <= '0';
        ...
        assert PCin = '1'
            report "JC no-carry failed: PCin should be 1"
            severity error;

        assert PCsel = "10"
            report "JC no-carry failed: PCsel should be 10, no jump"
            severity error;

        assert DTCM_wr = '0'
            report "JC no-carry failed: DTCM_wr should be 0"
            severity error;

        assert RFin = '0'
            report "JC no-carry failed: RFin should be 0"
            severity error;

        assert RFout = '0'
            report "JC no-carry failed: RFout should be 0"
            severity error;

        assert Cin = '0'
            report "JC no-carry failed: Cin should be 0"
            severity error;


        ----------------------------------------------------------------
        -- Step 3: Return to FETCH
        ----------------------------------------------------------------
        jc_s <= '0';
        ...
        assert IRin = '1'
            report "FETCH after JC no-carry failed: IRin should be 1"
            severity error;
        ...
        -- Finish
        ----------------------------------------------------------------
        report "JC no-carry test finished successfully" severity note;
