...
architecture sim of lab_3_tb_top is
  ...
  constant ITCM_INIT : word_array_t := (
    X"C100",   -- 00 : mov  r1, 0       ; r1 = &arr1
    X"C20E",   -- 01 : mov  r2, 14      ; r2 = &arr2
    X"C31C",   -- 02 : mov  r3, 28      ; r3 = &result
    X"C400",   -- 03 : mov  r4, 0       ; r4 = i (loop counter)
    X"C501",   -- 04 : mov  r5, 1       ; r5 = 1 (increment)
    X"C60E",   -- 05 : mov  r6, 14      ; r6 = N (loop bound)
    X"D710",   -- 06 : ld   r7, 0(r1)   ; LOOP: r7 = arr1[i]
    X"D820",   -- 07 : ld   r8, 0(r2)   ;        r8 = arr2[i]
    X"2945",   -- 08 : and  r9, r4, r5  ;        r9 = i & 1  (odd/even test)
    X"1B95",   -- 09 : sub  r11, r9, r5 ;        r11 = r9 - 1, sets C-flag
    X"9003",   -- 10 : jnc  +3          ;        if r9 < 1 (even) -> skip to ADD
    X"0A78",   -- 11 : add  r10, r7, r8 ;        ODD:  r10 = arr1[i] + arr2[i]
    X"EA30",   -- 12 : st   r10, 0(r3)  ;              result[i] = r10
    X"7002",   -- 13 : jmp  +2          ;              skip EVEN path
    X"1A78",   -- 14 : sub  r10, r7, r8 ;        EVEN: r10 = arr1[i] - arr2[i]
    X"EA30",   -- 15 : st   r10, 0(r3)  ;              result[i] = r10
    X"0115",   -- 16 : add  r1, r1, r5  ;        r1++
    X"0225",   -- 17 : add  r2, r2, r5  ;        r2++
    X"0335",   -- 18 : add  r3, r3, r5  ;        r3++
    X"0445",   -- 19 : add  r4, r4, r5  ;        i++
    X"1A46",   -- 20 : sub  r10, r4, r6 ;        r10 = i - N, sets C-flag
    X"90F1",   -- 21 : jnc  -15         ;        if i < N -> PC=7 (back to LOOP)
    X"F000",   -- 22 : done             ; halt
    X"0000",   -- 23 : nop              ; unreachable
    X"70FE"    -- 24 : jmp  -2          ; unreachable safety net
  );
  ...
  constant DTCM_INIT : word_array_t := (
    -- arr1[0..13]  @ DTCM[0..13]
    X"003F",   -- 00 : arr1[0]  = 63
    X"021E",   -- 01 : arr1[1]  = 542
    X"00F5",   -- 02 : arr1[2]  = 245
    X"00BE",   -- 03 : arr1[3]  = 190
    X"005B",   -- 04 : arr1[4]  = 91
    X"0056",   -- 05 : arr1[5]  = 86
    X"004E",   -- 06 : arr1[6]  = 78
    X"0040",   -- 07 : arr1[7]  = 64
    X"0053",   -- 08 : arr1[8]  = 83
    X"0010",   -- 09 : arr1[9]  = 16
    X"0018",   -- 10 : arr1[10] = 24
    X"003E",   -- 11 : arr1[11] = 62
    X"004F",   -- 12 : arr1[12] = 79
    X"0013",   -- 13 : arr1[13] = 19
    -- arr2[0..13]  @ DTCM[14..27]
    X"000D",   -- 14 : arr2[0]  = 13
    X"0138",   -- 15 : arr2[1]  = 312
    X"008D",   -- 16 : arr2[2]  = 141
    X"00A0",   -- 17 : arr2[3]  = 160
    X"005C",   -- 18 : arr2[4]  = 92
    X"0058",   -- 19 : arr2[5]  = 88
    X"0047",   -- 20 : arr2[6]  = 71
    X"003F",   -- 21 : arr2[7]  = 63
    X"003B",   -- 22 : arr2[8]  = 59
    X"000E",   -- 23 : arr2[9]  = 14
    X"002B",   -- 24 : arr2[10] = 43
    X"000C",   -- 25 : arr2[11] = 12
    X"0047",   -- 26 : arr2[12] = 71
    X"005A",   -- 27 : arr2[13] = 90
    -- result[0..13] @ DTCM[28..41]  (zero-init, filled by the loop)
    X"0000",   -- 28 : result[0]
    X"0000",   -- 29 : result[1]
    X"0000",   -- 30 : result[2]
    X"0000",   -- 31 : result[3]
...
begin
  ...
  begin
    ...
    for i in 0 to DEPT-1 loop
      ...
      wait until rising_edge(clk);
      hwrite(Lout, DTCM_tb_out);        -- 4 hex chars
