...
begin
  ...
  rtype_s <= add_s or sub_s or and_s or or_s or xor_s or shl_s; --real time
    ...
    ----------------------
      ...
      case state_reg is
        ...
        when S_Decode2_EX =>
          ...
          elsif shl_s = '1' then ALUFN <= "0101";   --shl
          end if;

          next_state <= S_WB;

        
        when S_WB => --write back for R tyoe
        ...
        when S_LDST_EX2 =>
          ...
          if ld_s = '1' then      --because its ld & st here
            next_state <= S_LD_EX3;    -- Wait for DTCM registered output to settle just for the ld op
          else
            next_state <= S_ST_EX3;
          end if;
